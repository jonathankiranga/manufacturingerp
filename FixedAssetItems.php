...
/* $Id: FixedAssetItems.php 6497 2013-12-14 00:41:44Z daintree $ */
		...
		if ($_POST['submit']==_('Update')) { /*so its an existing one */
			...
			if ($OldDetails['assetcategoryid'] !=$_POST['AssetCategoryID']  AND $OldDetails['cost']!=0){
				...
				$TransNo = GetNextTransNo( 42, $db); /* transaction type is asset category change */
                                $periodno = GetPeriod(ConvertSQLDate(Date('Y-m-d')),$db,false);
                                ...
                                $SQL =sprintf("INSERT INTO [Generalledger]
                                           ...
                                           [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
                                            '%s','%s','%s','%s','%s','%s','%s')", $JOURNAL,Date('Y-m-d'),$periodno,
                                            $TransNo,'42',$NewAccounts['costact'],$OldDetails['costact'] ,$OldDetails['cost'],
                                            $_POST['currency'],1,(($row['acctype']=='debtors')?$row['itemcode']:''),
                                            (($row['acctype']=='creditors')?$row['itemcode']:''),(($row['acctype']=='bank')?$row['itemcode']:''),0,
                                             ($AssetID . ' ' . _('change category') . ' ' . $OldDetails['assetcategoryid'] . ' - ' . $_POST['AssetCategoryID']),'0','','0',$_POST['DimensionOne'],$_POST['DimensionTwo'] );
                ...
                if ($OldDetails['accumdepn']!=0) {
                        ...
                        $SQL =sprintf("INSERT INTO [Generalledger]
                                 ...
                                 [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
                                  '%s','%s','%s','%s','%s','%s','%s')", $JOURNAL,Date('Y-m-d'),$periodno,
                                  $TransNo,'42',$OldDetails['accumdepnact'],$NewAccounts['accumdepnact'],$OldDetails['accumdepn'],
                                  $_POST['currency'],1,(($row['acctype']=='debtors')?$row['itemcode']:''),
                                  (($row['acctype']=='creditors')?$row['itemcode']:''),(($row['acctype']=='bank')?$row['itemcode']:''),0,
                                   ($AssetID . ' ' . _('change category') . ' ' . $OldDetails['assetcategoryid'] . ' - ' . $_POST['AssetCategoryID']),'0','','0',$_POST['DimensionOne'],$_POST['DimensionTwo'] );
		...
		if ($AssetRow['cost'] > 0){
			//credit cost for the asset deleted
                                ...
                                $SQL =sprintf("INSERT INTO [Generalledger]
                                           ...
                                           [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
                                            '%s','%s','%s','%s','%s','%s','%s')", $JOURNAL,Date('Y-m-d'),$periodno,
                                            $TransNo,'43',$AssetRow['accumdepnact'],$AssetRow['costact'],$Asset['accumdepn'],
                                            $_POST['currency'],1,(($row['acctype']=='debtors')?$row['itemcode']:''),
                                            (($row['acctype']=='creditors')?$row['itemcode']:''),(($row['acctype']=='bank')?$row['itemcode']:''),0,
                                             ( _('Delete asset') . ' ' . $AssetID ),'0','','0',$_POST['DimensionOne'],$_POST['DimensionTwo'] );
...
echo '<form autocomplete="off"id="AssetForm" enctype="multipart/form-data" method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">
      <div>';
...
if (isset($AssetRow['disposaldate']) AND $AssetRow['disposaldate'] !=null){
	echo '<tr>
...
if (isset($AssetRow) AND ($AssetRow['datepurchased']!=null AND $AssetRow['datepurchased']!='')){
	echo '<tr>
