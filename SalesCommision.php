...
if(isset($_POST['Save_Rate'])){
 ...
 . " FROM Commision where ceiling='%s' and rownumber='%s'",$_POST['Financial_Periods'],$rownumber),$db);
  ...
  $ResultIndex=DB_System("Select top 12 [periodno],[lastdate_in_period] as year "
    ...
    foreach ($ReportFinaceArray as $key=>$value) {
        ...
        $ResultIndex=DB_query("SELECT TOP 5 commisionabove,commisionbelow,rownumber,ceiling"
                . " FROM Commision where ceiling='".$value['periodno']."'  order by rownumber asc ",$db);
