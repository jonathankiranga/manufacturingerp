      ...
      if(isset($_SESSION[$mypage])){
        if($_SESSION[$mypage]==$_POST['TransID']){
            ...
            //Alter table Tasks add time_from varchar(10),time_to varchar(10),location varchar(max)
             ...
             VALUES ('%s',getdate(),getdate(),'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s')",
...
$SQL = array();
    if($status=='4'){
        $SQL1 = sprintf("Update [Tasks] set [Status]='%s',[taskdetails]='%s',lastactivity=getdate(),[frequency]='%s'"
        ...
        DB_query($SQL1,$db);
            ...
            if($frequencyint>0){
                  ...
                  $SQL2 = sprintf("INSERT INTO [Tasks]([userid],[datecreated],lastactivity,[TaskOwner],[Taskname],[datedue],[Status],
                   [Priority],[frequency],[taskdetails]) VALUES ('%s',getdate(),getdate(),'%s','%s','%s','%s','%s','%s','%s')",
