...
if(isset($_POST['Financial_Periods'])){
    ...
    $ResultIndex=DB_query("Select DATEADD(M,-1,[lastdate_in_period]+1) ,
             DATEADD(MINUTE,1439,[lastdate_in_period] )
  ...
  and SalesHeader.[salespersoncode]='%s' 
  and SalesHeader.documenttype=10  order by SalesHeader.docdate  DESC",$_POST['salespersoncode']);
...
Function selectlast(){
   ...
   $ResultIndex=DB_query("SELECT TOP 1 ceiling FROM Commision order by ceiling desc",$db);
   $rows = DB_fetch_row($ResultIndex);
   return $rows[0];
