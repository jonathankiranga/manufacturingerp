...
if(isset($_POST['trailbalance'])){
    ...
    $line_height = 12;
     ...
     $ResultsP = DB_query("EXEC [dbo].[AccountProject] @Yearindex='".$_POST['Financial_Periods']."'",$db);
     while($rows = DB_fetch_array($ResultsP)){
         
         $LeftOvers = $pdf->addTextWrap(40,$YPos,50, $FontSize, ucfirst($rows['Code']),'left');
         $LeftOvers = $pdf->addTextWrap(100,$YPos,100, $FontSize, ucfirst($rows['BudgetName']),'left');
    ...
    $pdf->OutputD($_SESSION['DatabaseName'] . '_' ._('Projects'). '_' . date('Y-m-d').'.pdf');
    $pdf->__destruct();
  ...
  echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/maintenance.png" title="' . _('Project Report') .'" alt="" />' . _('Project Report') . '</p>';
  ...
  $FR->Get();
  echo '<tr><td colspan="2"><input type="submit" name="trailbalance" value="Print Budgets by Project"/></td></tr></table>';
  echo '</div></form>';
