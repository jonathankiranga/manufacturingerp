...
if(isset($_POST['submit']) and $_POST['submit']=='Receive Goods'){
    include('purchases/SaveFassetGRN.inc');  
}
...
echo '<a href="'.htmlspecialchars('PurchaseAssetsList.php',ENT_QUOTES,'UTF-8').'">Go to Purchase orders list</a>';
...
if(isset($_GET['line'])) {
   $results = DB_query("Delete from [FixedAssetsLine] where [entryno]='".$_GET['line']."'",$db);
}
...
echo '<table  class="table table-bordered"><thead><tr>'
        ...
        . '<th class="number">Qty to<br />Recieve</th>'
        . '<th class="number">Qty to<br />Expect</th>'
        . '<th class="number">Location</th>'
        . '<th class="number">Invoice Price per unit</th>'
...
if($IsTaxed==true){
  ...
  if($VATinclusive == true){
    ...
    echo sprintf('<tr>'
         ...
         .'<td><a href="'.$pge.'?line='.$itemcode.'&ref='.$_POST['documentno'].'"  onclick="return confirm(\''._('Are you sure you wish to remove '.trim($stocklist['description']).' from this GRN ?').'\');">Remove Line</a></td>'
         .'</tr>',trim($stocklist['description']), createlocations($itemcode));
}
