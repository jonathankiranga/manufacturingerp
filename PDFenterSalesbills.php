...
$SQL="SELECT 
 ...
 join debtors on debtors.itemcode=EnterbillHeaders.VendorID 
...
Preparecolums(1);
  ...
  while($Row = DB_fetch_array($ResultIndex)){
        ...
        $pdf->addText($collumrows[$I]+ 300,$YPos ,$FontSize, number_format($Row['grossamount'],2));
         ...
         $pdf->addTextwrap($collumrows[$I],$YPos,200,$FontSize,$accdesc['accdesc']);
         $pdf->addText($collumrows[$I]+ 200,$YPos ,$FontSize, number_format($Row['vatamount']* -1,2));
         $pdf->addText($collumrows[$I]+ 300,$YPos ,$FontSize, number_format($Row['grossamount']* -1,2));
         $pdf->addTextwrap($Page_Width-$Right_Margin-100,$Page_Height-$Top_Margin-$FontSize * 4,100,$FontSize,_('Credit Note'),'right',0,1);
        }
...
echo '<form>
    ...
    <thead>'
      ...
      $SQL='SELECT TOP 1000 
         ...
         [narration],[journalno],VendorID,debtors.customer
         FROM [EnterbillHeaders] join debtors on debtors.itemcode=EnterbillHeaders.VendorID 
         order by EnterbillHeaders.[date] desc';
      $ResultIndex=DB_query($SQL, $db);
      while($row=DB_fetch_array($ResultIndex)){
           echo sprintf('<tr>'
            . '<td>%s</td>'
...
function Pageheader(){
 ...
 Global $firstrowpos,$lastrow,$narrative,$amount,$itemcode,$db,$date ;
...
$pdf->addTextwrap($Page_Width-$Right_Margin-120,$Page_Height-$Top_Margin-$FontSize * 3,120,$FontSize,_('Sales Bill No').':'.$docno,'right',0,1);
$pdf->addTextwrap($Page_Width-$Right_Margin-100,$Page_Height-$Top_Margin-$FontSize * 4,100,$FontSize,_('Date').':'.$date,'right');
...
$sqlcreditors=DB_query("SELECT ([customer]+[curr_cod]),[middlen],[phone],[email],[city] FROM debtors where itemcode='".$itemcode."'", $db);
