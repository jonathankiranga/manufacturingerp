...
echo '<div class="centre"><p class="page_title_text">'
. '<img src="'.$RootPath.'/css/'.$Theme.'/images/sales.png" title="' . _('Sales Loading List') .'" alt="" />' 
. ' ' . _('Sales Loading List') . '</p>';
 
if(isset($_POST['submit'])){ 
    if($_POST['submit']=='Confirm Loading Details'){
        include('transactions/SaveLoading.inc');  
    }
}
...
foreach ($Rowdata as $key => $stocklist) {
    ...
    $cnetamount=0; $cgrossamount=0; $emptyunits=0;$ifdone=0;
    ...
    $qtyreceived = $stocklist['modified'];
    ...
    if(isset($qty)){
        ...
        $qtytoinvoice = $qtyorderd-$qtyreceived;
    }
...
echo '<tfoot><tr>'
        ...
        . '<td colspan="3" class="number"><input type="submit" name="submit" value="' . _('Confirm Loading Details') . '"'
        . ' onclick="return confirm(\''._('Are you sure you wish to print this Loading List ?').'\');" /></td>'
        . '</tr></tfoot>';
echo '</table>';
echo '</div></div></form><code>WARNING<BR/>BEFORE YOU ISSUE A LOADING ORDER CONFIRM HOW MUCH IS ACTUALLY GOING TO BE LOADED.<BR/>This process cannot be corrected once done</code>';
