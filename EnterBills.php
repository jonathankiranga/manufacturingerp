...
echo '<p class="page_title_text"><img src="'.$RootPath.'/css/'.$Theme.'/images/supplier.png" title="' . _('Enter Bills') .'" alt="" />' . ' ' . _('Enter Bills') . '</p>';
...
echo '<input type="hidden" name="FormID" value="'. $_SESSION['FormID'] .'" />';
    ...
    if(isset($_POST['Savebill'])){
        ...
        $JouClass = new journalentries();
         ...
         if(DB_error_no($db)>0){
            ...
            'PDFenterbills.php',$_POST['documentno'], _('Print Enter Bills') .' '. $_POST['documentno']);
    ...
    if(isset($_GET['new'])){
        ...
        $_POST['documentno'] = GetNextTransNo(23,$db);
         }
    ...
    $ResultIndex = DB_query('Select getdate()+1 as date ',$db);
    $rowdate = DB_fetch_row($ResultIndex);
...
echo '<td>Document No<input tabindex="4" type="text" name="documentno" value="'.$_POST['documentno'].'"  size="10" maxlength="10" required="required" readonly="readonly"/></td>'
   . '<td>Supplier Reference</td><td><input tabindex="5" type="text" name="reference" value="'.$_POST['reference'].'"  size="10"  maxlength="10"/></td></tr>';

if(isset($_GET['SupplierID'])){
      $ResultIndex = DB_query("SELECT itemcode,customer,[curr_cod] "
    . "from creditors where itemcode='".$_GET['SupplierID']."'", $db);
...
echo '<tr><td><input tabindex="4" type="hidden" name="VendorID" id="VendorID" value="'.$_POST['VendorID'].'"  size="5"/>'
   . '<input type="button" id="searchvendor" value="Search for Vendor"/></td>';

echo '<td>Supplier Name</td>'
     . '<td><input tabindex="5" type="text" name="VendorName" id="VendorName" value="'.$_POST['VendorName'].'"  size="50"  required="required"/></td>';

echo '<td>Currency Code</td><td>'
    . '<input tabindex="6" type="text"  size="5" name="currencycode" id="currencycode" value="'.$_POST['currencycode'].'" readonly="readonly"/></td></tr>';
...
echo '</tr><tr><td>Auto Calculate VAT</td><td><select name="calculatevat">'
 ...
 echo $_POST['calculatevat']==0?'selected="selected"':"";
    echo '>No</option>'
    . '</select></td><td>Withhold Tax</td><td colspan="2"><select name="withholdtax">'
    . '<option value="1" '; 
 echo $_POST['withholdtax']==1?'selected="selected"':""; 
 echo '>Yes</option>'
    . '<option value="0" '; 
 echo $_POST['withholdtax']==0?'selected="selected"':"";
    echo '>No</option>'
    . '</select></td></tr>';
echo '<tr><td>Narration:</td>'
...
class journalentries {
    ...
    function GetSupplierCurrency($itemcode){
        ...
        $ResultIndex = DB_query("SELECT 
                    ...
                    [arpostinggroups].[VATinclusive],
                    [arpostinggroups].[IsTaxed]
                    FROM [creditors] left join arpostinggroups on [supplierposting]=[arpostinggroups].code 
        ...
        if(is_null($postingGroups['creditorsaccount']) and mb_strlen($itemcode)>2){
            prnMsg('Please set up a posting group for this supplier','warn');
        }
    ...
    Function SaveJournal(){
        Global $db;
         ...
         $this->JournalArray[] = sprintf("INSERT INTO [EnterbillHeaders]
          ...
          FormatDateForSQL($_POST['date']),($_SESSION['Grossamounttotal']>0?'23':'21'),$_POST['documentno'],
        ...
        $this->JournalArray[] = sprintf("INSERT INTO [SupplierStatement]
         ([Date],[Documentno] ,[Documenttype] ,[Accountno],[Grossamount] ,[JournalNo],[Dimension_One],[Dimension_Two],[Currency],whtax) VALUES
         ('%s','%s','%s' ,'%s' ,%f ,'%s' ,'%s' ,'%s' ,'%s','%s')",
         FormatDateForSQL($_POST['date']),$_POST['documentno'],($_SESSION['Grossamounttotal']>0?'23':'21'),$_POST['VendorID'],(0-$_SESSION['Grossamounttotal']),
         $JOURNAL,$_POST['DimensionOne'],$_POST['DimensionTwo'],$_POST['currencycode'],$_POST['withholdtax']);
                 
        $this->JournalArray[] = sprintf("INSERT INTO [creditorsledger]
        ([date],[details],[flag],[invref] ,[acctfolio],[amount],[type],[curr_cod],[curr_rat],[i_n_t],[period],[journal],[typ],[systypes_1],[ledger],whtax)
         VALUES ('%s','%s','%s','%s','%s',%f,'%s','%s',%f,'%s','%s','%s','%s','%s','%s','%s')",FormatDateForSQL($_POST['date']),
         $_POST['reference'].$_POST['comments'],'CR',$_POST['documentno'],$_POST['VendorID'],(0-$_SESSION['Grossamounttotal']),'P',$_POST['currencycode'],
         $Conrate,'J',$periodno,$JOURNAL,'P',($_SESSION['Grossamounttotal']>0?'23':'21'),$Cust['creditorsaccount'],$_POST['withholdtax']);
       ...
       foreach ($this->JOURNAL as $key => $row)  {
           ...
           [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
            ...
            $_POST['documentno'],'23',$row['account'],$Cust['creditorsaccount'],$row['Grossamount'],
           ...
           [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
            '%s','%s','%s','%s','%s','%s','%s')", $JOURNAL,FormatDateForSQL($_POST['date']),$periodno,
            $_POST['documentno'],'21',$Cust['creditorsaccount'],$row['account'],abs($row['Grossamount']),
            $_POST['currencycode'], $Conrate,'',$_POST['VendorID'],'',0,$_POST['reference'].$_POST['comments'],'0','','0',$_POST['DimensionOne'],$_POST['DimensionTwo'] );
           ...
           if(mb_strlen($row['defaultgl_vat'])>2){
               ...
               if($row['vatamount']>0){
                $this->JournalArray[]= sprintf("INSERT INTO [Generalledger]
                 ([journalno],[Docdate],[period],[DocumentNo],[DocumentType],[accountcode],
                  ...
                  [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
                  '%s','%s','%s','%s','%s','%s','%s')", $JOURNAL,FormatDateForSQL($_POST['date']),$periodno,
                  $_POST['documentno'],'23',$row['defaultgl_vat'],$row['account'],$row['vatamount'],
                 ...
                 ([journalno],[Docdate],[period],[DocumentNo],[DocumentType],[accountcode],
                  ...
                  [dimension],[dimension2]) VALUES ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s','%s',
                  '%s','%s','%s','%s','%s','%s','%s')", $JOURNAL,FormatDateForSQL($_POST['date']),$periodno,
                  $_POST['documentno'],'21',$row['account'],$row['defaultgl_vat'],abs($row['vatamount']),
                  $_POST['currencycode'], $Conrate,'',$_POST['VendorID'],'',0,
                  $_POST['reference'].$_POST['comments'],'0','','0',$_POST['DimensionOne'],$_POST['DimensionTwo'] );
               }
              ...
              if($row['vatamount']>0){
                   ...
                   ([documenttype],[documentno],[journalno],[account],[vatamount],[grossamount])
                    VALUES ('%s','%s' ,'%s' ,'%s' ,%f ,%f)",($_SESSION['Grossamounttotal']>0?'23':'21')
                   , $_POST['documentno'],$JOURNAL,$row['account'],$row['vatamount'],$row['Grossamount']);
              } else {
                    $this->JournalArray[] = sprintf("INSERT INTO [EnterbillsLines]
                   ([documenttype],[documentno],[journalno],[account],[vatamount],[grossamount])
                    VALUES ('%s','%s' ,'%s' ,'%s' ,%f ,%f)",($_SESSION['Grossamounttotal']>0?'23':'21')
                   , $_POST['documentno'],$JOURNAL,$row['account'],0,$row['Grossamount']);  
              }
