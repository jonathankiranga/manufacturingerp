...
/*
Function Nothing(){
    'http://www.oanda.com/convert/fxdaily?value=1&redirected=1&exch=USD&format=CSV&dest=Get+Table&sel_list=KES' ;
...
LOAD Currency,
     ...
     [Units/1 USD]
FROM
[http://www.oanda.com/currency/table?date=08/11/14&date_fmt=us&exch=USD&sel_list=KES&value=1&format=CSV&redirected=1]
 * 

http://www.google.com/finance/converter?a=1&from=KES&to=USD
}
*/
...
Function Num2Wrd($amount,$currencysymbol,$centname){
    $cents = '';
    // php_intl.dll needs to be enabled in the iis
    $arrayamount = explode('.',$amount);
   
    $results = new NumberFormatter('en_US',NumberFormatter::SPELLOUT,NumberFormatter::TYPE_CURRENCY);
    $shillings = $results->formatCurrency($arrayamount[0],'KES');

    $trimCents =substr($arrayamount[1],0,2);
    
    if($trimCents>0){
      $cents  =' and ';
      $cents .= trim( $results->formatCurrency($trimCents,'KES'));
      $cents .=' '. $centname;
    }

    return strtoupper(trim($currencysymbol).' '.trim($shillings) .' '.$cents.' ONLY');
