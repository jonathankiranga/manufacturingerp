...
$POSclass = new NewAssemble();
 ...
 if(!isset($_POST['date'])){ 
    $ResultIndex = DB_query('Select getdate()-30 as date ',$db);
  ...
  $SQL=sprintf("Select 
    ...
    echo '<table class="table-bordered" id="testTable">'
      ...
      if(IsTankOrStore(trim($StockID))=="S"){
            $sql="Select sum(fulqty * [stockledger].[partperunit])+isnull(sum(loosqty),0) as Total,
                    [lunit].[descrip] 
                    from [stockledger] 
     ...
     echo sprintf('<tr>'
       ...
       if(IsTankOrStore(trim($StockID))=="S"){
           $sql="Select
            ...
            loosqty,
            (fulqty * [stockledger].[partperunit])+isnull(loosqty,0) as Total
             from [stockledger] 
