...
<?php
  ...
  If(isset($_POST['Submit'])){
    ...
    $PeriodNo = GetPeriod($_POST['date'],$db);
     include('includes/CustomersAgeingHeader.inc');
         $age1=0;$age2=0;$age3=0;$age4=0;$age5=0;
         
         $SqlOutput=DB_query("Select itemcode,customer from debtors  where [curr_cod]='".$_POST['curr_cod']."' order by customer asc",$db);
    ...
    $pdf->OutputD($_SESSION['DatabaseName'].'_' ._('CustAgeing').'_'.$_POST['date'].'.pdf');
    $pdf->__destruct();
    ...
    $csv_output = "'Customer Ageing Analysis As at','".$_POST['date']."'\n";
    $csv_output .= "'Customer ID','Customer Name','Current','60 days','90 days','120 days','over 120 days'\n";
    ...
    $FileName =  $_SESSION['reports_dir'] . '/CustomerAgeing_' .date('Y-m-d').'.csv';
    $csvFile = fopen($FileName, 'w');
    $i = fwrite($csvFile, $csv_output);
    header('Location: ' . $_SESSION['reports_dir'] . '/CustomerAgeing_' .date('Y-m-d').'.csv');
    ...
    echo '<div class="responsive"><table class="table-bordered"><tr><td>As At</td><td><input tabindex="1" type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="date" size="11" maxlength="10" autofocus="autofocus" required="required" value="' .$_POST['date']. '" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')"/></tr>';
...
function Ageing($custid,$period){
 ...
 //From 'INV1661' has no duplicates   
 $ageingarray=array() ;
 
 $SQL="EXEC [dbo].[ageingCustomersReport] @account ='".$custid."',@Month ='".$period."'";
 $ResultIndex = DB_query($SQL,$db);
 $agerow = DB_fetch_row($ResultIndex);
 
 $ageingarray[0]= is_null($agerow[0])?0:$agerow[0];
 $ageingarray[1]= is_null($agerow[1])?0:$agerow[1];
 $ageingarray[2]= is_null($agerow[2])?0:$agerow[2];
 $ageingarray[3]= is_null($agerow[3])?0:$agerow[3];
 $ageingarray[4]= is_null($agerow[4])?0:$agerow[4];
