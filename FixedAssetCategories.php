...
if (isset($_POST['submit'])) {
	...
	$SQL = "SELECT [PostingGroup],[Makeinactive],[Fluctuation] FROM [BankAccounts]";
	...
	if (isset($SelectedCategory) AND $InputError !=1) {
		...
		$sql = "UPDATE fixedassetcategories
                                SET categorydescription = '" . $_POST['CategoryDescription'] . "',
                                        ...
                                        [Equipment_hired_act]= '" . $_POST['Equipment_hired_act'] . "',
                                        [defaultgl_vat_act]= '" . $_POST['defaultgl_vat_act'] . "',
                                        [vatcategorycode]= '" . $_POST['vatcategorycode'] . "'
		...
		$sql = "INSERT INTO fixedassetcategories (categoryid,
                        ...
                        VALUES ('" . $_POST['CategoryID'] . "',
                                        ...
                                        '" . $_POST['DisposalAct'] . "',
                                        '" . $_POST['AccumDepnAct'] . "',
                                        '" . $_POST['Equipment_hired_act'] . "',
                                        '" . $_POST['defaultgl_vat_act'] . "',
                                        '" . $_POST['vatcategorycode'] . "')";
	...
	unset($_POST['AccumDepnAct']);
        unset($_POST['Equipment_hired_act']);
        unset($_POST['defaultgl_vat_act']);
        unset($_POST['vatcategorycode']);
...
or deletion of the records*/
	...
	echo '<tr>
			...
			<th>' . _('Description') . '</th>
			<th>' . _('Asset Cost GL') . '</th>
			<th>' . _('P &amp; L Depn GL') . '</th>
			<th>' . _('Disposal GL') . '</th>
			<th>' . _('Accum Depn GL') . '</th>
                        <th>' . _('P &amp; L On-Hire GL') . '</th>
                        <th>' . _('VAT On-Hire GL') . '</th>
                        <th>' . _('VAT Category for On-Hire ') . '</th>
                        <th>Edit </th>
                        <th>Delete</th>
		  </tr>';
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		printf('<td>%s</td>
                        ...
                        $myrow['disposalact'],
                        $myrow['accumdepnact'],
                        $myrow['Equipment_hired_act'],
                        $myrow['defaultgl_vat_act'],
                        $myrow['vatcategorycode'],
...
echo '<form autocomplete="off"id="CategoryForm" method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'],ENT_QUOTES,'UTF-8') . '">';
...
if (isset($SelectedCategory) and !isset($_POST['submit'])) {
	//editing an existing fixed asset category
		...
		$myrow = DB_fetch_array($result);
                ...
                $_POST['Equipment_hired_act'] = $myrow['Equipment_hired_act'];
                $_POST['defaultgl_vat_act'] = $myrow['defaultgl_vat_act'];
                $_POST['vatcategorycode'] = $myrow['vatcategorycode'];
...
//SQL to poulate account table table-bordered boxes
$sql = "Select accno as accountcode,accdesc as accountname from acct where balance_income=0 and [ReportStyle]=0";
$BSAccountsResult = DB_query($sql,$db);

$sql = "Select accno as accountcode,accdesc as accountname from acct where balance_income=1  and [ReportStyle]=0";
...
echo '</select></td></tr>';
	...
	DB_data_seek($PnLAccountsResult,0);
echo '<tr><td>' . _('Profit and Loss On-Hire GL Code') . ':</td>
	<td><select name="Equipment_hired_act" required="required" title="' . _('Select the general ledger account where the Income from Hire  on assets of this category should be posted to. Only Profit and loss accounts can be selected') . '" >';

while ($myrow = DB_fetch_array($PnLAccountsResult)) {

	if (isset($_POST['Equipment_hired_act']) and $myrow['accountcode']==$_POST['Equipment_hired_act']) {
		echo '<option selected="selected" value="'.$myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'],ENT_QUOTES,'UTF-8',false) . ' ('.$myrow['accountcode'].')' . '</option>';
	} else {
		echo '<option value="'.$myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'],ENT_QUOTES,'UTF-8',false) . ' ('.$myrow['accountcode'].')' . '</option>';
	}
...
DB_data_seek($BSAccountsResult,0);
echo '<tr><td>' . _('Balance Sheet VAT GL Code') . ':</td>
         <td><select name="defaultgl_vat_act" required="required" title="' . _('Select the general ledger account where the Hire-Income VAT on assets of this category should be posted to. Only balance sheet accounts can be selected') . '" >';

while ($myrow = DB_fetch_array($BSAccountsResult)) {

	if (isset($_POST['defaultgl_vat_act']) and $myrow['accountcode']==$_POST['defaultgl_vat_act']) {
		echo '<option selected="selected" value="'.$myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'],ENT_QUOTES,'UTF-8',false) . ' ('.$myrow['accountcode'].')' . '</option>';
	} else {
		echo '<option value="'.$myrow['accountcode'] . '">' . htmlspecialchars($myrow['accountname'],ENT_QUOTES,'UTF-8',false) . ' ('.$myrow['accountcode'].')' . '</option>';
	}
...
echo '</select></td>
	</tr><tr><td>Select VAT Category</td><td><select name="vatcategorycode"><option></option>';
    $result=DB_query("Select vatc,vat from vatcategory",$db);
   
    
    while ($myrow = DB_fetch_array($result)) {
        if ($_POST['vatcategorycode']==trim($myrow['vatc'])){
