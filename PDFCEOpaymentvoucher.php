...
Function Paymentlist($journalno){
  ...
  while($rows=DB_fetch_array($ResultIndex)){
    $invoice_journal = SelectSupplierInvoiceNo($rows['invoice_journal']);
    ...
    $YPos -= $line_height;
     ...
     $LeftOvers = $pdf->addTextWrap(45,  $YPos,100, $FontSize,$invoice_journal,'left');
...
If(isset($paymentvoucherno)){
 ...
 include('includes/header.inc');
  ...
  FROM [paymentvoucherheader] join [creditors]
  on [paymentvoucherheader].[itemcode]=[creditors].[itemcode]
  where ([paymentvoucherheader].[status]=1)
   order by date desc" ;
  ...
  while($row=DB_fetch_array($ResultIndex)){
      ...
      echo sprintf('<td><a href="%s">CEO Approve</a></td></tr>','ApproveCEOVoucher.php?edit='.$row['journal'],$row['docno']);
