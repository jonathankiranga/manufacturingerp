...
if(isset($_POST['salesreport'])){
    ...
    $R1=0; $R2=0; $R3=0; $Visa=0; $ledger=0; $mpesa=0; $cash=0;
     ...
     $SQL=Sprintf("SELECT 
      ...
      ,sum(SalesLine.invoiceamount-SalesLine.vatamount) as netamt
  FROM [SalesHeader] join SalesLine 
        ...
        where SalesHeader.[docdate] between '%s' and '%s' 
        group by 
       [SalesHeader].[documentno]
      ,[SalesHeader].[userid]
      ,[SalesHeader].[docdate]
      ,[SalesHeader].[status]
      ,[SalesHeader].[customercode]
      ,[SalesHeader].[customername]",$castdate,$Tcastdate);
     ...
     while($rows = DB_fetch_array($Results)){
         ...
         $custArray = GetCustomerdetails($rows['customercode']);
     ...
     $LeftOvers = $pdf->addTextWrap(240, $YPos,150, $FontSize, _('Total VAT Sales'),'right');
     $LeftOvers = $pdf->addTextWrap(485, $YPos, 85, $FontSize, number_format($R1 ,2),'right');
     $YPos -= $line_height ;
    
     $LeftOvers = $pdf->addTextWrap(240, $YPos,150, $FontSize, _('Credit Notes(NET)'),'right');
     $LeftOvers = $pdf->addTextWrap(485, $YPos, 85, $FontSize, number_format($R2,2),'right');
     
    $pdf->OutputD('VAT' . '_CreditNotesEport_' . $castdate . '_' . date('Y-m-d').'.pdf');
    $pdf->__destruct();
...
echo '<table class="table table-bordered"><tr>'
  ...
  echo '<tr><td colspan="2"><input type="submit" name="salesreport" value="Print Sales credite Notes Report"/></td></tr>'
  . '</table>';
  echo '</div></form>';
...
function salesHeader(){
    ...
    Global $line_height,$Bottom_Margin,$FontSize,$XPos,$R1,$R2,$R3,$castdate,$Tcastdate;
   ...
   $pdf->newPage();
       ...
       $pdf->addText(240,$YPos,10,_('Daily Sales Credit Note Summary ').':'.$reportdate.' to '. $Treportdate);        
       $YPos -= $line_height * 2;
...
function GetCustomerdetails($CustomerID){
    ...
    $sqldebtors=DB_query("SELECT [itemcode],[middlen] as PIN,[customer],[phone],
    [email],[city],[country],[curr_cod],[customerposting],[salesman],[VATinclusive]
    FROM [debtors] join postinggroups on code=[customerposting]  where itemcode='".$CustomerID."'", $db);
