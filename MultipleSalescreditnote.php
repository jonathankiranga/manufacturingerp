...
if(isset($_POST['confirm']) && $_SESSION['DocumentPosted']==false){
    if($_SESSION['Grossamounttotal']>0){
          include('transactions/SavevatcreditNote.inc');
    }else{
...
if(isset($_POST['ref'])){
    $_SESSION['documentno'] = $_POST['ref'];
    $_SESSION['DocumentPosted']=false;
}
 
foreach ($_SESSION['documentno'] as $key => $value) {
    $_POST['documentno']=$key;
    break;
}
...
echo '<tr><td>Date</td><td><input tabindex="1" type="text" class="date" alt="'.$_SESSION['DefaultDateFormat'].'" name="date" size="11" maxlength="10"  value="' .$_POST['date']. '" onchange="isDate(this, this.value, '."'".$_SESSION['DefaultDateFormat']."'".')"/></td>';

echo '<td>KRA Reference</td><td><input tabindex="1" type="text" name="reference" value="'.$_POST['reference'].'" required="required"/></td></tr>';
...
$runningvattotal  = 0;
$runninggrosstotal  = 0;
foreach ($_SESSION['documentno'] as  $documentno => $value ) {
...
$Slaqry ="SELECT 
 ...
 where [documentno]='".$documentno."' 
     group by  
      ...
      ,[locationcode],[code] ,[description] ,[unitofmeasure],[Quantity]
...
while($stocklist = DB_fetch_array($ResultIndex)){
    ...
    echo sprintf('<tr>'
            .'<td>%s</td>'
         .'<td><input type="text" name="code['.$itemcode.']" value="'.$stkcode.'"  readonly="readonly"/></td>'
         .'<td>%s</td>'
         .'<td>%s</td>'
         .'<td class="number"><input type="text" class="integer" name="subunits['.$itemcode.']" value="'.$qty.'"  /></td>'
         .'<td class="number"><input type="text" class="number" name="salesprice['.$itemcode.']" value="'.$salesprice.'" /></td>'
         .'<td class="number"><input type="text" class="number" name="netamount['.$itemcode.']" value="'.$netamount.'" readonly="readonly" /></td>'
         .'<td class="number"><input type="text" class="number" name="vatamount['.$itemcode.']" value="'.$vatamount.'" readonly="readonly" /></td>'
         .'<td class="number"><input type="text" class="number" name="grossamount['.$itemcode.']" value="'.$grossamount.'" readonly="readonly" /></td>'
         .'</tr>','INV:'.$documentno,trim($stocklist['description']),$stocklist['unitofmeasure'].'(1 x'. number_format($stocklist['partperunit'],0).')');
}
}
...
echo '</table>';
echo '<div class="centre"><input type="submit" name="submit" value="' . _('Re-Calculate') . '" /><input type="submit" name="confirm" value="' . _('Enter Credit Note Details and Return VAT') . '"
            onclick="return confirm(\''._('Are you sure you wish to Return VAT ?').'\');" /></div>';
