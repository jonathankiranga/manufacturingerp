...
$Title = _('Run Maintenance Reminders');
...
$sql="SELECT 	description,
				taskdescription,
				DATEADD(day,frequencydays,lastcompleted) AS duedate,
				userresponsible,
				email
		...
		WHERE DATEADD(day,frequencydays-10,lastcompleted) > getdate()
...
$sql="SELECT 	description,
		...
		INNER JOIN www_users
		ON fixedassettasks.userresponsible=www_users.userid
		WHERE DATEADD(day,frequencydays,lastcompleted)> GETDATE()
		ORDER BY manager";
