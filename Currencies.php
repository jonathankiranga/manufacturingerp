<?php
/* $Id: Currencies.php 6496 2013-12-12 02:48:15Z rchacon $*/
...
if (isset($_POST['submit'])) {
	...
	if (isset($SelectedCurrency) AND $InputError !=1) {
		...
		$MYSQL[] = "UPDATE currencies SET	country='". $_POST['Country']. "',
	...
	/*Selected currencies is null cos no item selected on first time round so must be adding a record must be submitting new entries in the new payment terms form */
		$MYSQL[] = "INSERT INTO currencies (currency,
                                                currabrev,
                                                country,
                                                hundredsname,
                                                decimalplaces,
	...
	if (isset($SelectedCurrency) AND $InputError !=1) {
		...
		$SQLBankAccounts = "SELECT bankName, 
					accountcode,
                                        PostingGroup,
                                        Fluctuation
					FROM BankAccounts
		...
		while ($myrowBankAccount=DB_fetch_array($resultBankAccounts)){
			...
			$SQL = "SELECT sum([amount]) AS balance
					...
					WHERE  bankcode='" . $myrowBankAccount['accountcode'] . "'";
			...
			if(OldRate != NewRate){
                            ...
                            $MYSQL[]=sprintf("INSERT INTO [Generalledger]
                                                        ([journalno]
                                                        ,[Docdate]
                                                        ,[period]
                                                        ,[DocumentNo]
                                                        ...
                                                        ,[ExchangeRateDiff])
                                                  VALUES
                                    ('%s','%s','%s','%s','%s','%s','%s',%f,'%s',%f,'%s','%s',%f)",
                                    $ExDiffTransNo,
                                    FormatDateForSQL($PostingDate),
                                    $PeriodNo,$ExDiffTransNo,36,
                                    $myrowBankAccount['PostingGroup']
                                    ,$myrowBankAccount['Fluctuation'],
                                    $DifferenceToAdjust,$SelectedCurrency,
                                    $myrowBankAccount['accountcode'],
                                    ($myrowBankAccount['bankName'] . ' ' . _('currency rate adjustment to') . ' ' . locale_number_format($NewRate,8) . ' ' . $SelectedCurrency . '/' . $_SESSION['CompanyRecord']['currencydefault'])
                                    ,(OldRate-NewRate));
...
// PREVENT DELETES IF DEPENDENT RECORDS IN DebtorsMaster
	...
	$sql= "SELECT COUNT(*) FROM debtors	WHERE curr_cod= '" . $SelectedCurrency . "'";
	$result = DB_query($sql,$db);
	$myrow = DB_fetch_row($result);
	if ($myrow[0] > 0)
	{
		...
		$sql= "SELECT COUNT(*) FROM creditors WHERE curr_cod = '".$SelectedCurrency."'";
		$result = DB_query($sql,$db);
		$myrow = DB_fetch_row($result);
		if ($myrow[0] > 0) {
			...
			$sql= "SELECT COUNT(*) FROM BankAccounts WHERE currency = '" . $SelectedCurrency . "'";
			$result = DB_query($sql,$db);
			$myrow = DB_fetch_row($result);
			if ($myrow[0] > 0){
