...
if(isset($_POST['salesreport'])){
  ...
  FROM [SalesHeader] join SalesLine 
        ...
        and SalesHeader.documenttype=10
     ...
     $LeftOvers = $pdf->addTextWrap(240, $YPos,150, $FontSize, _('NET Sales'),'right');
     $LeftOvers = $pdf->addTextWrap(485, $YPos, 85, $FontSize, number_format($R2,2),'right');
     
    $pdf->OutputD('SALESVAT' . '_SalesREport_' . $castdate . '_' . date('Y-m-d').'.pdf');
    $pdf->__destruct();
...
echo '<table class="table table-bordered"><tr>'
  ...
  echo '<tr><td colspan="2"><input type="submit" name="salesreport" value="Print Sales Report"/></td></tr>'
  . '</table>';
  echo '</div></form>';
...
function salesHeader(){
   ...
   $pdf->newPage();
       ...
       $pdf->addText(240,$YPos,10,_('Daily Sales VAT Report ').':'.$reportdate.' to '. $Treportdate);        
       $YPos -= $line_height * 2;
